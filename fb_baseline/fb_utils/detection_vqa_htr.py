...
class CrossAttentionLayer(nn.Module):
    ...
    def __init__(self, hid_dim, n_heads, pf_dim, dropout=0.15):
        super().__init__()
...
def vqa_evaluation(model, images, tokens, attention_masks, max_answer_length):
    patchs = rearrange(images, 'b c (h p1) (w p2) -> b (w) (h) (p1 p2 c)',
                  p1=model.vqa_config['patch_h'], p2=model.vqa_config['patch_w'])
    patchs = patchs.squeeze(2)
    patchs = model.vqa_input_image_layer(patchs)
...
def detection_evaluation(model, images, input_ids, attention_masks, cor_treshhold, treshhold):
    patchs = rearrange(images, 'b c (h p1) (w p2) -> b (w) (h) (p1 p2 c)',
                  p1=model.detection_config['patch_h'], p2=model.detection_config['patch_w'])
    patchs = patchs.squeeze(2)
    patchs = model.detection_input_image_layer(patchs)
    gpt_img = model.gpt_model(inputs_embeds=patchs).last_hidden_state
...
def htr_evaluation(model, images, attention_mask, tokens, max_answer_length):
    patchs = rearrange(images, 'b c (h p1) (w p2) -> b (w) (h) (p1 p2 c)',
                      p1=model.handwritten_config['patch_h'], p2=model.handwritten_config['patch_w'])
    patchs = patchs.squeeze(2)
    patchs = model.handwritten_input_layer(patchs)
    
    answer_logits = []
    bs = images.shape[0]
    for _ in range(max_answer_length):
        tokens_emb = model.gpt_model.wte(tokens)
        input_emb = torch.cat((patchs, tokens_emb), dim=1)
        gpt_text = model.gpt_model(inputs_embeds=input_emb, attention_mask=attention_mask).last_hidden_state
        ...
        new_tokens = torch.multinomial(last_logits.softmax(-1), num_samples=1)
        tokens = torch.cat([tokens, new_tokens], dim=-1)
        new_attention_mask = torch.ones(bs, 1).to(attention_mask.device, dtype=attention_mask.dtype)
